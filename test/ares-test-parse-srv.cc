...
TEST_F(LibraryTest, ParseCSrvReplyOK) {
  DNSPacket pkt;
  pkt.set_qid(0x1234).set_response().set_aa()
    .add_question(new DNSQuestion("example.com", T_SRV))
    .add_answer(new DNSSrvRR("example.com", 100, 10, 20, 30, "srv.example.com"))
  ...
  EXPECT_EQ(21, cares_srv_reply_get_weight(srv2));
  EXPECT_EQ(31, cares_srv_reply_get_port(srv2));
  EXPECT_EQ(100, cares_srv_reply_get_ttl(srv2));
  EXPECT_EQ(nullptr, cares_srv_reply_get_next(srv2));
  ares_free_data(srv);
}
...
TEST_F(LibraryTest, ParseCSrvReplySingle) {
  DNSPacket pkt;
  pkt.set_qid(0x1234).set_response().set_aa()
    .add_question(new DNSQuestion("example.abc.def.com", T_SRV))
    .add_answer(new DNSSrvRR("example.abc.def.com", 180, 0, 10, 8160, "example.abc.def.com"))
  ...
  EXPECT_EQ(180, cares_srv_reply_get_ttl(srv));
  EXPECT_EQ(nullptr, cares_srv_reply_get_next(srv));
...
TEST_F(LibraryTest, ParseCSrvReplyMalformed) {
  std::vector<byte> data = {
    0x12, 0x34,  // qid
    0x84, // response + query + AA + not-TC + not-RD
    0x00, // not-RA + not-Z + not-AD + not-CD + rc=NoError
...
TEST_F(LibraryTest, ParseCSrvReplyMultiple) {
  DNSPacket pkt;
  pkt.set_qid(0x1234).set_response().set_ra().set_rd()
    .add_question(new DNSQuestion("srv.example.com", T_SRV))
    .add_answer(new DNSSrvRR("srv.example.com", 300, 0, 5, 6789, "a1.srv.example.com"))
  ...
  EXPECT_EQ(300, cares_srv_reply_get_ttl(srv));
...
TEST_F(LibraryTest, ParseCSrvReplyCname) {
  DNSPacket pkt;
  pkt.set_qid(0x1234).set_response().set_aa()
    .add_question(new DNSQuestion("example.abc.def.com", T_SRV))
    .add_answer(new DNSCnameRR("example.abc.def.com", 300, "cname.abc.def.com"))
...
TEST_F(LibraryTest, ParseCSrvReplyCnameMultiple) {
  DNSPacket pkt;
  pkt.set_qid(0x1234).set_response().set_ra().set_rd()
    .add_question(new DNSQuestion("query.example.com", T_SRV))
    .add_answer(new DNSCnameRR("query.example.com", 300, "srv.example.com"))
...
TEST_F(LibraryTest, ParseCSrvReplyErrors) {
  ...
  data = pkt.data();
  for (size_t len = 1; len < data.size(); len++) {
    int rc = cares_parse_srv_reply(data.data(), len, &srv);
    EXPECT_TRUE(rc == ARES_EBADRESP || rc == ARES_EBADNAME);
  }
}
...
TEST_F(LibraryTest, ParseCSrvReplyAllocFail) {
  DNSPacket pkt;
  pkt.set_qid(0x1234).set_response().set_aa()
    .add_question(new DNSQuestion("example.abc.def.com", T_SRV))
    .add_answer(new DNSCnameRR("example.com", 300, "c.example.com"))
  ...
  for (int ii = 1; ii <= 5; ii++) {
    ClearFails();
    SetAllocFail(ii);
    EXPECT_EQ(ARES_ENOMEM, cares_parse_srv_reply(data.data(), data.size(), &srv)) << ii;
  }
}
