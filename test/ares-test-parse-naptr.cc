...
TEST_F(LibraryTest, ParseCNaptrReplyOK) {
  DNSPacket pkt;
  pkt.set_qid(0x1234).set_response().set_aa()
    .add_question(new DNSQuestion("example.com", T_NAPTR))
    .add_answer(new DNSNaptrRR("example.com", 100,
  ...
  EXPECT_EQ("service", std::string((char*)cares_naptr_reply_get_service(naptr)));
  EXPECT_EQ("regexp", std::string((char*)cares_naptr_reply_get_regexp(naptr)));
  EXPECT_EQ("replace", std::string((char*)cares_naptr_reply_get_replacement(naptr)));
  EXPECT_EQ(10, cares_naptr_reply_get_order(naptr));
  ...
  EXPECT_EQ("regexp2", std::string((char*)cares_naptr_reply_get_regexp(naptr2)));
  EXPECT_EQ("replace2", std::string((char*)cares_naptr_reply_get_replacement(naptr2)));
  EXPECT_EQ(11, cares_naptr_reply_get_order(naptr2));
  EXPECT_EQ(21, cares_naptr_reply_get_preference(naptr2));
  EXPECT_EQ(nullptr, cares_naptr_reply_get_next(naptr2));
  EXPECT_EQ(16, cares_naptr_reply_get_ttl(naptr2));
...
TEST_F(LibraryTest, ParseCNaptrReplyErrors) {
  DNSPacket pkt;
  pkt.set_qid(0x1234).set_response().set_aa()
    .add_question(new DNSQuestion("example.com", T_NAPTR))
    .add_answer(new DNSNaptrRR("example.com", 100,
  ...
  pkt.add_question(new DNSQuestion("Axample.com", T_NAPTR));
  data = pkt.data();
  EXPECT_EQ(ARES_ENODATA, cares_parse_naptr_reply(data.data(), data.size(), &naptr));
  pkt.questions_.clear();
  pkt.add_question(new DNSQuestion("example.com", T_NAPTR));
#endif
  ...
  data = pkt.data();
  for (size_t len = 1; len < data.size(); len++) {
    int rc = cares_parse_naptr_reply(data.data(), len, &naptr);
    EXPECT_TRUE(rc == ARES_EBADRESP || rc == ARES_EBADNAME);
  }
}
...
TEST_F(LibraryTest, ParseCNaptrReplyTooShort) {
  std::vector<byte> data = {
    0x12, 0x34,  // qid
    0x84, // response + query + AA + not-TC + not-RD
    0x00, // not-RA + not-Z + not-AD + not-CD + rc=NoError
...
TEST_F(LibraryTest, ParseCNaptrReplyAllocFail) {
  DNSPacket pkt;
  pkt.set_qid(0x1234).set_response().set_aa()
    .add_question(new DNSQuestion("example.com", T_NAPTR))
    .add_answer(new DNSNaptrRR("example.com", 100,
  ...
  for (int ii = 1; ii <= 13; ii++) {
    ClearFails();
    SetAllocFail(ii);
    EXPECT_EQ(ARES_ENOMEM, cares_parse_naptr_reply(data.data(), data.size(), &naptr));
  }
}
