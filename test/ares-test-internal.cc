...
extern "C" int single__domain(ares_channel, const char*, char**);
TEST_F(DefaultChannelTest, SingleDomain) {
  ...
  char *ptr = nullptr;
  EXPECT_EQ(ARES_ENOMEM, single__domain(channel_, "www", &ptr));

  channel_->flags |= ARES_FLAG_NOSEARCH|ARES_FLAG_NOALIASES;
  EXPECT_EQ(ARES_SUCCESS, single__domain(channel_, "www", &ptr));
